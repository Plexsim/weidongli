...
class jsModuleSite extends WeModuleSite {
	public $headtable='js_buildpro_head';
	public $subtable='js_buildpro_sub';
	public $housetalbe='js_buildpro_house';
	public $felltable='js_buildpro_fell';
	public $fellrecordtable='js_buildpro_fell_record';
	public $experttable='js_buildpro_expert_comment';
	public $albumtable='js_buildpro_album';
	public $fullviewtable='js_buildpro_full_view';
	public $billtable='js_buildpro_bill';
	...
	public function doWebAdd() {
		...
		if (checksubmit('submit')) {
			...
			message('俱乐部信息更新成功！', referer(), 'success');
	...
	public function doWebBuildsubadd () {
		...
		if (checksubmit('submit')) {
			...
			message('子项目信息更新成功！', $this->createWebUrl('buildsubadd', array('sid'=>$sid,'hid' =>$hid )), 'success');
	...
	public function doWebHouseadd () {
		...
		if (checksubmit('submit')) {
			...
			message('子项目信息更新成功！', $this->createWebUrl('houseadd', array('hsid'=>$hsid,'hid'=>$hid)), 'success');
	...
	public function doWebAlbumadd () {
		...
		if (checksubmit('submit')) {
			...
			message('健身相册信息更新成功！', $this->createWebUrl('albumlist', array('hid'=>$hid)), 'success');
	...
	public function doWebExpertadd() {
		...
		if (!empty($id)&& !empty($hid)) {
			...
			if (empty($item)) {
				message('抱歉，该会员点评不存在或是已经删除！', '', 'error');
			}
		}
		...
		if (empty($id))
			...
			message('会员点评信息更新成功！', $this->createWebUrl('expertlist', array('hid'=>$hid)), 'success');
	...
	public function doWebFullviewadd() {
		...
		if (checksubmit('submit')) {
			...
			message('全景图片更新成功！', $this->createWebUrl('fullviewlist', array('hsid'=>$hsid)), 'success');
	...
	public function doWebDelbuild()
	{
		...
		if (empty($item)) {
			message('抱歉，该俱乐部不存在或是已经删除！', '', 'error');
		}
		if (!empty($item['pic'])) {
	...
	public function doWebDelsubbuild()
	{
		...
		if (empty($item)) {
			message('抱歉，该子项目不存在或是已经删除！', '', 'error');
		}
	...
	public function doMobileHuxing() {
		global $_GPC, $_W;
		$hid = $_GPC['hid'];
		$item = pdo_fetchall("SELECT * FROM ".tablename($this->housetalbe)." WHERE weid=:weid and hid = :hid order by sid desc,sort desc", array(':weid'=>$_W['weid'],':hid' => $hid));
		$res=array();		
		foreach ($item as $itv)
		{			
			$nowsid=$itv['sid'];
		...
		if((!empty($weid)&&(!empty($hid))))
		{
			if($type=='getpic'){
				...
				foreach($xcres as $xc) {
					...
					//第一行
					$line1data = array();
					$line1data[] = array('type'=>'title','title'=>$xc['title'],'subTitle'=>$xc['title']);
					for($i=0;$i<$mhsl;$i++){
						$pic = $pics[$i];
...
<panorama id="" hideabout="1">
	...
	public function geturl($type=1)
	{
		...
		{
			case 1:
				$img_url='./source/modules/js/template/img/build_home.png';
				break;
			case 2:
				$img_url='./source/modules/js/template/img/build_banner.png';
				break;
			case 3:
				$img_url='./source/modules/js/template/img/build_house_banner.png';
				break;
			case 4:
				$img_url='./source/modules/js/template/img/YouGotMe.mp3';
				break;
			default:
				$img_url='./source/modules/js/template/img/art_pic.png';
		}
		return $img_url;			
	}
	...
	public function getindexpic($picurl)
	{
		...
		if(empty($picurl))
		{
			$img_url=$_W['siteroot'].'source/modules/js/template/img/index.jpg';
		}
		else 
		{
	...
	public function getdefaultfullviewpic($type,$picurl)
	{
		...
		switch ($type)
		{
			case 1:
				if(empty($picurl))
				{
					$img_url=$_W['siteroot'].'source/modules/js/template/img/qian.jpg';
				}
				else 
				{
					$img_url=$_W['attachurl'].$picurl;
			...
			case 2:
				if(empty($picurl))
				{
				$img_url=$_W['siteroot'].'source/modules/js/template/img/hou.jpg';
				}
				else 
				{
					$img_url=$_W['attachurl'].$picurl;
			...
			case 3:
				if(empty($picurl))
				{
				$img_url=$_W['siteroot'].'source/modules/js/template/img/zuo.jpg';
				}				
				else
				{
					$img_url=$_W['attachurl'].$picurl;
			...
			case 4:
				if(empty($picurl))
				{
				$img_url=$_W['siteroot'].'source/modules/js/template/img/you.jpg';
				}				
				else
				{
					$img_url=$_W['attachurl'].$picurl;
			...
			case 5:
				if(empty($picurl))
				{
					$img_url=$_W['siteroot'].'source/modules/js/template/img/shang.jpg';
				}				
				else
				{
					$img_url=$_W['attachurl'].$picurl;
			...
			case 6:
				if(empty($picurl))
				{
					$img_url=$_W['siteroot'].'source/modules/js/template/img/xia.jpg';
				}				
				else
				{
					$img_url=$_W['attachurl'].$picurl;
