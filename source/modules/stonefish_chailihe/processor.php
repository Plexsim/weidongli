...
class stonefish_chailiheModuleProcessor extends WeModuleProcessor {
	public $name = 'stonefish_chailiheModuleProcessor';
	...
	public function respond() {
		...
		$weid = $_W['weid'];//当前公众号ID
				...
				$row = pdo_fetch($sql, array(':rid' => $rid));
					...
					//查询是否被屏蔽
					$lists = pdo_fetch("SELECT status,zhongjiang FROM ".tablename($this->table_list)." WHERE from_user = '".$from."' and weid = '".$weid."' and rid= '".$rid."' limit 1" );
					...
					if($now >= $row['start_time'] && $now <= $row['end_time']){						
						if ($row['status']==0){
						    ...
						    return $this->respNews(array(
							    ...
							    'PicUrl' => $picture,
							    'Url' => $this->createMobileUrl('chailihe', array('rid' => $rid, 'from_user' => base64_encode(authcode($this->message['from'], 'ENCODE')))),
						    ));
						}
					}else{
