...
abstract class WeModuleProcessor {
	...
	protected function respText($content) {
		...
		//$response['Content'] = htmlspecialchars_decode($content)//原版
		//增加by 超级无聊 20140721 小尾巴模块
		//start
		$response['Content'] = htmlspecialchars_decode($content).$this->respTail();
		//end
		return $response;
	}
	...
	protected function respNews(array $news) {
		...
		//增加by 超级无聊 20140721 小尾巴模块
		//start
		if($response['ArticleCount']<9){
			$response['Articles'][]=$this->respTail('news');
		}
		//end
	...
	//增加by 超级无聊 20140721
	//start
	protected function respTail($type='basic'){
		//判断小尾巴是否开启
		$set = pdo_fetch("SELECT * FROM ".tablename('tail_set')." WHERE weid = :weid", array(':weid' => $GLOBALS['_W']['weid']));
		...
		if($type=='basic'){
			if($set['itype']==1){
				$item=pdo_fetch("SELECT id,content,times FROM ".tablename('tail_list')." WHERE weid = :weid AND module='basic' order by times asc", array(':weid' => $GLOBALS['_W']['weid']));
				
			}else{
				$item=pdo_fetch("SELECT *  FROM `".tablename('tail_list')."` AS t1 JOIN (SELECT ROUND(RAND() * ((SELECT MAX(id) FROM `".tablename('tail_list')."`)-(SELECT MIN(id) FROM `".tablename('tail_list')."`))+(SELECT MIN(id) FROM `".tablename('tail_list')."`)) AS id) AS t2 WHERE t1.id >= t2.id ORDER BY t1.id LIMIT 1;");
			}
			...
			//更新次数
			pdo_update('tail_list',array('times'=>$item['times']+1),array('id'=>$item['id']));
			return "\n".$item['content'];
		
		}elseif($type=='news'){
			if($set['itype']==1){
				$item=pdo_fetch("SELECT * FROM ".tablename('tail_list')." WHERE weid = :weid AND module='news' order by times asc", array(':weid' => $GLOBALS['_W']['weid']));
			}else{
				$item=pdo_fetch("SELECT *  FROM `".tablename('tail_list')."` AS t1 JOIN (SELECT ROUND(RAND() * ((SELECT MAX(id) FROM `".tablename('tail_list')."`)-(SELECT MIN(id) FROM `".tablename('tail_list')."`))+(SELECT MIN(id) FROM `".tablename('tail_list')."`)) AS id) AS t2 WHERE t1.id >= t2.id AND weid = :weid AND module='news' ORDER BY t1.id LIMIT 1", array(':weid' => $GLOBALS['_W']['weid']));
			}
			...
			$return = array(
				...
				'PicUrl' => !empty($item['thumb']) && !strexists($item['thumb'], 'http://') ? $GLOBALS['_W']['attachurl'] . $item['thumb'] : $item['thumb'],
				'Url' => $this->buildSiteUrl($item['url']),
				'TagName' => 'item',
			);
			pdo_update('tail_list',array('times'=>$item['times']+1),array('id'=>$item['id']));
			return $return;
