...
class We7_photomakerModuleSite extends WeModuleSite {
	...
	public function doWebDevice() {
		...
		header('Location: rule.php?act=post&module=we7photomaker');
	...
	public function doWebGetSetting() {
		...
		$device = pdo_fetch("SELECT * FROM ".tablename('we7photomaker')." WHERE sn = :sn", array(':sn' => $sn));
	...
	public function doWebGetData() {
		...
		$device = pdo_fetch("SELECT enablemsg, leavemsg, rid, token FROM ".tablename('we7photomaker')." WHERE sn = :sn", array(':sn' => $sn));
		...
		$list = pdo_fetchall("SELECT id, leavemsg, photo FROM ".tablename('we7photomaker_log')." WHERE status = 1 AND rid = '{$device['rid']}' LIMIT 50");
	...
	public function doWebFinished() {
		...
		pdo_update('we7photomaker_log', array('status' => 2), array('id' => $id));
	...
	public function doWebFailed() {
		...
		pdo_update('we7photomaker_log', array('status' => 3), array('id' => $id));
