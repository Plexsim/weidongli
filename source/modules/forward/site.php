...
class forwardModuleSite extends WeModuleSite {	
	...
	public $table_data   = 'forward_data';
	...
	public function getHomeTiles($keyword = '') {
		...
		$urls = array();
		$list = pdo_fetchall("SELECT name, id FROM ".tablename('rule')." WHERE weid = ".$weid." and module = 'forward'".(!empty($keyword) ? " AND name LIKE '%{$keyword}%'" : ''));
		if (!empty($list)) {
			foreach ($list as $row) {
	...
	public function doWebSharelist($rid, $state) {		
		...
		if (checksubmit('delete')) {
			...
			message('删除成功！', create_url('site/module', array('do' => 'sharelist', 'name' => 'forward', 'id' => $id, 'page' => $_GPC['page'])));
		}
		$rules = pdo_fetchall('SELECT `id`,`name` FROM '.tablename('rule').' WHERE `module`=\'forward\'');
	...
	public function doWebShareranklist($rid, $state) {		
		...
		if (checksubmit('delete')) {
			pdo_delete($this->table_list, " id IN ('".implode("','", $_GPC['select'])."')");
			message('删除成功！', create_url('site/module', array('do' => 'shareranklist', 'name' => 'forward', 'id' => $id, 'page' => $_GPC['page'])));
		}
		$rules = pdo_fetchall('SELECT `id`,`name` FROM '.tablename('rule').' WHERE `module`=\'forward\'');
		$pindex = max(1, intval($_GPC['page']));
		$psize = 15;
	...
	public function doWebdos( $id = 0) {
		...
		message('屏蔽操作成功！', create_url('site/module/'.$sharelist.'', array('name' => 'forward', 'id' => $rid, 'page' => $_GPC['page'])));
	}	
	public function doWebSharedata($rid, $state) {		
		...
		if (checksubmit('delete')) {
			...
			message('删除成功！', create_url('site/module/sharedata', array('name' => 'forward', 'id' => $id, 'page' => $_GPC['page'])));
		}
		$rules = pdo_fetchall('SELECT `id`,`name` FROM '.tablename('rule').' WHERE `module`=\'forward\'');
		$pindex = max(1, intval($_GPC['page']));
		$psize = 15;
	...
	public function doWebdeldata( $id = 0) {
		...
		if (!empty($id)) {
			...
			message('删除成功！', create_url('site/module/sharedata', array('name' => 'forward', 'id' => $rid, 'page' => $_GPC['page'])));
		}		
