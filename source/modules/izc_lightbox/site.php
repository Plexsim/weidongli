...

{
    ...
    public $modulename = 'izc_lightbox';
...

    {
        ...
        pdo_update('izc_lightbox_list', array('hits' => $list['hits'] + 1), array('id' => $list['id']));

        $items = pdo_fetchall('SELECT * FROM' . tablename('izc_lightbox_page') . ' WHERE `list_id`=:list_id ORDER BY `listorder` desc,id asc', array(':list_id' => $list['id']));
...

    {
        ...
        $list = pdo_fetch('SELECT * FROM' . tablename('izc_lightbox_list') . ' WHERE `id`=:id  AND `weid`=:weid', array(':weid' => $_W['weid'], ':id' => $id));
...

    {
        ...
        pdo_query('update ' . tablename('izc_lightbox_list') . ' set shares=shares+1 where id=' . $id);
...

    {
        ...
        if ($foo == 'setstatus') {
            ...
            pdo_update('izc_lightbox_book', array('status' => $_GPC['status']), array('id' => intval($_GPC['book_id'])));
            ...
            pdo_delete('izc_lightbox_book', array('id' => $id, 'weid' => $_W['account']['weid']));
            ...
            foreach ($ids as $k => $v) {
                ...
                pdo_delete('izc_lightbox_book', array('id' => $v, 'weid' => $_W['account']['weid']));
        ...
        $list = pdo_fetchall('SELECT * FROM' . tablename('izc_lightbox_book') . ' WHERE `weid`=:weid AND list_id=:list_id LIMIT ' . ($pindex - 1) * $psize . ',' . $psize, array(':weid' => $_W['account']['weid'], ':list_id' => $id));

        $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('izc_lightbox_book') . ' WHERE `weid`=:weid AND list_id=:list_id LIMIT ' . ($pindex - 1) * $psize . ',' . $psize, array(':weid' => $_W['account']['weid'], ':list_id' => $id));
        ...
        if (!empty($s_id)) {
            ...
            $app = pdo_fetch('select iden,title from ' . tablename('izc_lightbox_app') . ' where id=' . $s_id . '');
        ...
        if ($foo == 'create') {
            ...
            if ($step == 1) {
                ...
                if ($_W['isfounder']) {
                    ...
                    $list = pdo_fetchall('select * from ' . tablename('izc_lightbox_app') . ' where 1=1 ' . $condition . ' order by id desc');

                } else {

                    $list = pdo_fetchall('select * from ' . tablename('izc_lightbox_app') . ' where isshow=1 ' . $condition . ' order by id desc');
                ...
                $app = pdo_fetch('select iden,title,price,author,series,thumb,qrcode from ' . tablename('izc_lightbox_app') . ' where id=' . $s_id . '');
                ...
                $isallow = pdo_fetch('select status,appnums from ' . tablename('izc_lightbox_manage') . '  where weid=:weid AND appid=:appid', array(':weid' => $_W['weid'], ':appid' => $s_id));

                if ($isallow['status'] == 1) {

                    $used = pdo_fetchcolumn('select count(id) from ' . tablename('izc_lightbox_list') . ' where weid=:weid AND s_id=:s_id', array(':weid' => $_W['weid'], ':s_id' => $s_id));
            ...
            pdo_delete('izc_lightbox_page', array('list_id' => $id));
...

    {
        ...
        if ($id > 0) {
            ...
            $item = pdo_fetch('select * from ' . tablename('izc_lightbox_list') . ' where id=:id AND weid=:weid', array(':weid' => $_W['weid'], ':id' => $id));
        ...
        if ($_W['ispost']) {
            ...
            $fields = array('weid', 'title', 'cover', 'cover1', 'cover2', 'reply_title', 'reply_thumb', 'share_thumb', 'reply_description', 'share_title', 'share_content', 'share_cb_url', 'share_cb_tel', 'first_type', 'first_btn_select', 'first_btn_url', 'first_btn_tel', 'bg_music_switch', 'bg_music_url', 'bg_music_icon', 'cover_title', 'cover_subtitle', 'tongji');
            ...
            if ($item == false) {
                ...
                $temp = pdo_insert('izc_lightbox_list', $insert);

            } else {

                $temp = pdo_update('izc_lightbox_list', $insert, array('id' => $item['id']));
        ...
        if ($item == false) {
            ...
            $item = array('reply_thumb' => $_W['siteroot'] . 'source/modules/izc_lightbox/style/img/default_cover.jpg', 'share_tips' => $_W['siteroot'] . 'source/modules/izc_lightbox/style/img/default_bg.jpg', 'cover' => $_W['siteroot'] . 'source/modules/izc_lightbox/style/img/default_bg.jpg', 'bg_music_url' => $_W['siteroot'] . 'source/modules/izc_lightbox/style/mp3/YouGotMe.mp3', 'bg_music_switch' => 1, 'bg_music_icon' => 1, 'start_time' => time(), 'end_time' => strtotime('+7 day'));
...

    {
        ...
        $sql = 'SELECT * FROM' . tablename('izc_lightbox_list') . "WHERE `id`={$id} AND weid={$_W['weid']}";
...

    {
        ...
        $item = pdo_fetch('select * from ' . tablename('izc_lightbox_list') . ' where id=:id AND weid=:weid', array(':weid' => $_W['weid'], ':id' => $list_id));

        $list = pdo_fetchall('select * from ' . tablename('izc_lightbox_page') . ' where list_id=:list_id AND weid=:weid order by listorder asc,id desc', array(':weid' => $_W['weid'], ':list_id' => $list_id));

        if ($item != false) {

            $app = pdo_fetch('select iden,title from ' . tablename('izc_lightbox_app') . ' where id=' . $item['s_id'] . '');
...

    {
        ...
        $list = pdo_fetch('select * from ' . tablename('izc_lightbox_list') . ' where id=:id AND weid=:weid', array(':weid' => $_W['weid'], ':id' => $list_id));

        if ($list != false) {

            $app = pdo_fetch('select iden,title from ' . tablename('izc_lightbox_app') . ' where id=' . $list['s_id'] . '');
...

    {
        ...
        if (!empty($kwd)) {
            ...
            $sql = 'SELECT id,reply_title,reply_thumb,reply_description FROM ' . tablename('izc_lightbox_list') . ' WHERE `weid`=:weid AND `title` LIKE :reply_title';
            ...
            foreach ($itype as $k => $v) {
                ...
                switch ($v) {
                    ...
                    case 1:
                        ...
                        pdo_update('izc_lightbox_items', array('video' => '', 'lat' => 0, 'lng' => 0, 'video_thumb' => '', 'address' => '', 'tel' => '', 'wechat' => '', 'map_thumb' => '', 'wurl' => ''), array('id' => $k, 'weid' => $_W['account']['weid']));
                    ...
                    case 3:
                        ...
                        pdo_update('izc_lightbox_items', array('lng' => $_GPC['lng'][$k], 'lat' => $_GPC['lat'][$k], 'address' => $_GPC['address'][$k], 'tel' => $_GPC['tel'][$k], 'wechat' => $_GPC['wechat'][$k], 'map_thumb' => $_GPC['map_thumb'][$k], 'wurl' => $wurl), array('id' => $k, 'weid' => $_W['account']['weid']));
...

    {
        ...
        if ($op == 'ok') {
            ...
            pdo_update('izc_lightbox_comment', array('status' => 1), array('id' => $id));
            ...
            pdo_delete('izc_lightbox_comment', array('id' => $id));
            ...
            $list = pdo_fetchall('SELECT * FROM ' . tablename('izc_lightbox_comment') . " WHERE list_id = '{$list_id}'  {$condition} ORDER BY  id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);

            $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('izc_lightbox_comment') . " WHERE list_id = '{$list_id}'  {$condition}");
            ...
            if (is_array($ids)) {
                ...
                foreach ($ids as $v) {
                    ...
                    if ($id > 0) {
                        ...
                        pdo_update('izc_lightbox_comment', array('status' => 1), array('id' => $id, 'list_id' => $_GPC['list_id']));
                ...
                pdo_update('izc_lightbox_comment', array('status' => 1), array('id' => $ids, 'list_id' => $_GPC['list_id']));
            ...
            if (is_array($ids)) {
                ...
                foreach ($ids as $v) {
                    ...
                    if ($id > 0) {
                        ...
                        pdo_delete('izc_lightbox_comment', array('id' => $id, 'list_id' => $_GPC['list_id']));
                ...
                pdo_delete('izc_lightbox_comment', array('id' => $ids, 'list_id' => $_GPC['list_id']));
...

    {
        ...
        if ($_W['isfounder']) {
            ...
            if ($_GPC['op'] == 'accredit') {
                ...
                $app = pdo_fetch('select * from ' . tablename('izc_lightbox_app') . "  where id={$appid}");
                ...
                $item = pdo_fetch('select * from ' . tablename('izc_lightbox_manage') . '  where weid=:weid AND appid=:appid', array(':weid' => $_W['weid'], ':appid' => $appid));
                ...
                if ($_W['ispost']) {
                    ...
                    if ($item == false) {
                        ...
                        $temp = pdo_insert('izc_lightbox_manage', $insert);

                    } else {

                        $temp = pdo_update('izc_lightbox_manage', $insert, array('id' => $item['id']));
                ...
                if ($id > 0) {
                    ...
                    $item = pdo_fetch('select * from ' . tablename('izc_lightbox_app') . "  where id={$id}");
                ...
                if ($_W['ispost']) {
                    ...
                    if ($item == false) {
                        ...
                        $temp = pdo_insert('izc_lightbox_app', $insert);

                    } else {

                        $temp = pdo_update('izc_lightbox_app', $insert, array('id' => $item['id']));
                ...
                if ($item == false) {
                    ...
                    $item = array('listorder' => 0, 'author' => '吾爱传媒', 'series' => '52media');
                ...
                $list_count = pdo_fetchcolumn('select count(*) from ' . tablename('izc_lightbox_list') . ' where s_id=' . $id);
                ...
                if ($list_count > 0) {
                    ...
                    $temp = pdo_delete('izc_lightbox_app', array('id' => $id));
                ...
                $app = pdo_fetch('select iden,title from ' . tablename('izc_lightbox_app') . "  where id={$appid}");
                ...
                $list = pdo_fetchall('select * from ' . tablename('izc_lightbox_app') . ' order by id desc LIMIT ' . ($pindex - 1) * $psize . ',' . $psize);

                $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('izc_lightbox_app') . "  {$condition}");
