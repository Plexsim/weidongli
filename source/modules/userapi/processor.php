...
class UserapiModuleProcessor extends WeModuleProcessor {
	...
	private function procRemote($item) {
		...
		if (!is_error($response)) {
			...
			if(strexists($result['Content'], '{begin-context}')) {

				$this->beginContext(3600);

				$result['Content'] = str_ireplace('{begin-context}', '', $result['Content']);

			}

			if(@stristr($result['Content'], '{end-context}') !== false) {

				$this->endContext();

				$result['Content'] = str_ireplace('{end-context}', '', $result['Content']);
