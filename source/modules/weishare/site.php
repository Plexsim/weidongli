...
class WeiShareModuleSite extends WeModuleSite
{
    ...
    public function doWebShare()
    {
        ...
        if ($operation == 'post') { // 添加
            ...
            if (checksubmit('submit')) {
                ...
                message('更新分享活动成功！', create_url('site/module/share', array(
                    'name' => 'weishare',
                    'op' => 'display'
                )), 'success');
            }
            ...
            $list = pdo_fetchall("SELECT * FROM " . tablename($this->table_share) . " WHERE weid = '{$_W['weid']}'  ORDER BY createtime DESC, id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
            $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename($this->table_share) . " WHERE weid = '{$_W['weid']}'");
    ...
    public function doWebShareUser()
    {
        ...
        if ($operation == 'post') { // 添加
            ...
            if (checksubmit('submit')) {
                ...
                message('更新领卡用户信息成功！', create_url('site/module/shareuser', array(
                'name' => 'weishare',
                'op' => 'display',
                'sid'=> $_GPC['sid']  
                    )), 'success');
            ...
            $list = pdo_fetchall("SELECT * FROM " . tablename($this->weishare_user) . " WHERE sid = '{$_GPC['sid']}'  ORDER BY income desc, createtime DESC, id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
            $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename($this->weishare_user) . " WHERE sid = '{$_GPC['sid']}'");
            $pager = pagination($total, $pindex, $psize);
        } elseif ($operation == 'delete') {
            $id = intval($_GPC['id']);
    ...
    public function doWebSetting()
    {
        ...
        $setting = pdo_fetch("SELECT * FROM " . tablename($this->weishare_authsetting)." where weid=:weid",array(":weid"=>$_W['weid']));
        ...
        if (checksubmit('submit')) {
            ...
            message('更新授权接口成功！', create_url('site/module/setting', array(
                'name' => 'weishare',
                'op' => 'display'
            )), 'success');
        }
    ...
    public function getopenid($code,$errorUrl)
    {
        ...
        $appid = $_W['account']['key'];
        $secret = $_W['account']['secret'];
        $setting = pdo_fetch("SELECT * FROM " . tablename($this->weishare_authsetting)." where weid=:weid",array(":weid"=>$_GPC['weid']));
    ...
    public function doMobilefirendshare()
    {
        ...
        if ($state==0) { // 第一步
            ...
            $url = $_W['siteroot'] . $this->createMobileUrl('firendshare', array(
                'sid' => $sid,
                'uid' => $uid
            ));
        ...
        }else if($state==1) { // token 换 accessToken
            ...
            $firdOpenId = $this->getopenid($code,$url); 
    ...
    public function doMobileShare()
    {
        ...
        if (empty($from_user)) {
            message('请先关注公众账号再注册领卡！', '', 'error');
            exit();
        }
        
        if (empty($code)) {
            $url = $_W['siteroot'] . $this->createMobileUrl('share', array(
                'id' => $id
            ));
    ...
    public function doMobileRegist()
    {
        ...
        if ($state==0) {
            $url = $_W['siteroot'] . $this->createMobileUrl('regist', array(
                'sid' => $sid,
                'tel' => $tel
            ));
            ...
            if(!empty($shareUser)){
                ...
                $code = $_GPC['code'];
                $openid = $this->getopenid($code,null);
                $share = pdo_fetch("SELECT * FROM " . tablename($this->table_share) . " WHERE id = :id", array(
                    ':id' => $sid
                ));
    ...
    public  function  doMobilesort(){
        ...
        $shareUser = pdo_fetch("SELECT * FROM " . tablename($this->weishare_user) . " WHERE sid = :sid order by income desc", array(
            ':sid' => $sid
        ));
    ...
    public function doMobileHelp()
    {
        ...
        if (empty($code)) { // 第一步
            ...
            $url = $_W['siteroot'] . $this->createMobileUrl('help', array(
                'sid' => $sid,
                'uid' => $uid
            ));
