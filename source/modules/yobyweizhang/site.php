...
class YobyweizhangModuleSite extends WeModuleSite {
	public function doMobileDetail() {//手机显示图片详细信息
		global $_GPC, $_W;
				$key=$_W['account']['modules']['yobyweizhang']['config']['kkk'];
				$city=$_W['account']['modules']['yobyweizhang']['config']['city'];
					$file2 = fopen(IA_ROOT.'/source/modules/yobyweizhang/citys.csv','r'); 
...
global $_GPC;
		$file3 = fopen(IA_ROOT.'/source/modules/yobyweizhang/citys.csv','r'); 
...
$arr3[] =$data3;
 ...
 foreach($arr3 as $v3){
    if($_GPC['city']==$v3[1]){
    $q3= array('isfa'=>$v3[2],'fa'=>$v3[3],'isc'=>$v3[4],'c'=>$v3[5]);
    }
 }
 echo json_encode($q3);
