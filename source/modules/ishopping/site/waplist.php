 ...
 */
    ...
    if($cateid == 0){
		...
		include $this->template('wap_list_all');
	} else {
		//获取当前分类
		$curcategory = pdo_fetch("SELECT * FROM ".tablename('ishopping_category')." WHERE weid = '{$_W['weid']}'  and id={$cateid}");
		if($curcategory == false) {
			message('分类错误，或者分类已经删除！');
		}
		if($curcategory['parentid'] != 0){
			$upcateory = pdo_fetch("SELECT * FROM ".tablename('ishopping_category')." WHERE weid = '{$_W['weid']}'  and id={$curcategory['parentid']}");
			if($upcateory == false) {
				message('分类错误，或者分类已经删除！');
			}
            $title = $upcateory['name'];
			$catename = 'ccate';
		} else {
		...
		include $this->template('wap_list_select');
	}