...
class LxybuscardModule extends WeModule {
	...
	public function fieldsFormDisplay($rid = 0) {
		...
		global $_W;
		$weid=$_W['weid'];
		$cards = pdo_fetchall("SELECT id,username FROM ".tablename($this->cardtable)." WHERE weid = :weid ORDER BY `id` DESC", array(':weid' => $weid));
		if (!empty($rid)) {
