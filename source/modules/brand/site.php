...
class BrandModuleSite extends WeModuleSite
{
    ...
    public function doWebBrand()
    {
        ...
        if ($operation == 'edit') { // 添加或编辑
            ...
            if (checksubmit('bname')) {
                ...
                message('更新微品牌成功！', create_url('site/module/brand', array(
                    'name' => 'brand',
                    'op' => 'display'
                )), 'success');
            }
            ...
            $list = pdo_fetchall("SELECT * FROM " . tablename($this->table_brand) . " WHERE weid = '{$_W['weid']}'  ORDER BY createtime DESC, id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
            $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename($this->table_brand) . " WHERE weid = '{$_W['weid']}'");
    ...
    public function doWebNote()
    {
        ...
        if ($operation == 'display') { // 说明项
            ...
            if (checksubmit('submit')) {
                ...
                message('更新活动说明项成功！', create_url('site/module/note', array(
                    'name' => 'brand',
                    'op' => 'display',
                    'bid' => $bid
                )), 'success');
    ...
    public function doWebImage()
    {
        ...
        if ($operation == 'display') { // 日程显示
            ...
            if (checksubmit('submit')) {
                ...
                message('更新图片项成功！', create_url('site/module/image', array(
                    'name' => 'brand',
                    'op' => 'display',
                    'bid' => $bid
                )), 'success');
    ...
    public function doWebProduct()
    {
        ...
        if ($operation == 'display') { // 日程显示
            ...
            if (checksubmit('submit')) {
                ...
                message('更新产品成功！', create_url('site/module/product', array(
                    'name' => 'brand',
                    'op' => 'display',
                    'bid' => $bid
                )), 'success');
