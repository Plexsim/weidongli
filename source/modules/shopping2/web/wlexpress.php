...
if($operation=='display'){
	$list = pdo_fetchall("SELECT * FROM ".tablename('shopping2_express')." WHERE weid = '{$_W['weid']}' ORDER BY displayorder DESC");
	...
	if (checksubmit('submit')) {
		...
		if (!empty($id)) {
			pdo_update('shopping2_express', $data, array('id' => $id));
		} else {
			pdo_insert('shopping2_express', $data);
			$id = pdo_insertid();
		}
	...
	if($id==0){
		...
		//修改
		$express = pdo_fetch("SELECT * FROM ".tablename('shopping2_express')." WHERE id = '$id'");
	}
	if (empty($express)) {
	...
	$id = intval($_GPC['id']);
	$express = pdo_fetch("SELECT id  FROM ".tablename('shopping2_express')." WHERE id = '$id' AND weid=".$_W['weid']."");
	if (empty($express)) {
		message('抱歉，物流方式不存在或是已经被删除！', $this->createWebUrl('express', array('op' => 'display')), 'error');
	}
	pdo_delete('shopping2_express', array('id' => $id));
