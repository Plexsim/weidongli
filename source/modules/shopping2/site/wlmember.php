...
if($_GPC['d']=='detail'){
	...
	$order = pdo_fetch("SELECT * FROM ".tablename('shopping2_order')." WHERE weid = '{$weid}'  $condition ");
	...
	$address=pdo_fetch("SELECT * FROM ".tablename('shopping2_address')." WHERE weid = '{$weid}' AND id={$order['aid']}");
}else{
	$pindex = max(1, intval($_GPC['page']));
	$psize = 10;
	$condition='';
	$condition="AND status ={$status} AND from_user='{$_W['fans']['from_user']}'";
	$list = pdo_fetchall("SELECT * FROM ".tablename('shopping2_order')." WHERE weid = '{$weid}'  $condition ORDER BY createtime DESC, id DESC LIMIT ".($pindex - 1) * $psize.','.$psize);
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('shopping2_order') . " WHERE weid = '{$weid}' $condition");
	$pager='';
	if($pindex==1){
	...
	foreach($list as $k=>$v){
		$list[$k]['goods']= pdo_fetchall("SELECT a.*,b.title,b.thumb,b.marketprice FROM ".tablename('shopping2_order_goods')." as a left join  ".tablename('shopping2_goods')." as b on a.goodsid=b.id WHERE a.weid = '{$weid}' and a.orderid={$v['id']} limit 3");
	}
}
