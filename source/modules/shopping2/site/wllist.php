 ...
 */
	...
	if($cateid==0){
		...
		$psize = 10;
		$condition='';
 		$list = pdo_fetchall("SELECT * FROM ".tablename('shopping2_goods')." WHERE weid = '{$weid}' AND status = '1' $condition ORDER BY displayorder DESC, id DESC LIMIT ".($pindex - 1) * $psize.','.$psize);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('shopping2_goods') . " WHERE weid = '{$weid}' AND status = '1' $condition");
		$pager='';
		if($pindex==1){
		...
		//获取当前分类
		$curcategory = pdo_fetch("SELECT * FROM ".tablename('shopping2_category')." WHERE weid = '{$weid}'  and id={$cateid}");
		if($curcategory==false){
			message('分类错误，或者分类已经删除！');
		}
		if($curcategory['parentid']!=0){		
			$upcateory=pdo_fetch("SELECT * FROM ".tablename('shopping2_category')." WHERE weid = '{$weid}'  and id={$curcategory['parentid']}");
			if($upcateory==false){
				message('分类错误，或者分类已经删除！');
			}
			$catename='ccate';
