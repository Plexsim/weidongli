...
class RedpacketModuleSite extends WeModuleSite
{
    ...
    public function doWebPacket()
    {
        ...
        if ($operation == 'post'){
            ...
            if (checksubmit('submit')){
                ...
                $data = array(
                    'weid' => $_W['weid'],
                    'packetsummary' => $_GPC["packetsummary"],
                ...
                message('更新红包有礼活动成功！', create_url('site/module/Packet', array(
                            'name' => 'redpacket',
                            'op' => 'display'
                            )), 'success');
                }
            ...
            $psize = 20;
            $list = pdo_fetchall("SELECT * FROM " . tablename($this -> table_redpacket) . " WHERE weid = '{$_W['weid']}'  ORDER BY createtime DESC, id DESC LIMIT " . ($pindex -1) * $psize . ',' . $psize);
            $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename($this -> table_redpacket) . " WHERE weid = '{$_W['weid']}'");
    ...
    public function doWebvirtualUser(){
        ...
        if ($operation == 'post'){
            ...
            if (checksubmit('submit')){
                ...
                $lowpoint = $awards[0]["point"];
                $status = 0;
                if($_GPC['income'] >= $lowpoint){
                    $status = 1;
                    }else{
                    $status = 0;
                    }
                ...
                message('添加虚拟用户成功！', create_url('site/module/fanslist', array(
                            'name' => 'redpacket',
                            'pid' => $pid,
                            )), 'success');
                }
    ...
    public function doMobileFirend()
    {
        ...
        $firend = $this -> getUserInfo($openid, $accessToken);
        $firendExists = false;
        ...
        $leftTotalCount = $totallimit - $firendHelpCount;
        $allow = true;
        if ($limitType == 0 && $leftTotalCount <= 0){
            $allow = false;
            $msg = "亲你的翻牌次数已用完!";
            }
    ...
    public function doMobileFirendFanpai()
    {
        ...
        if (empty($packet)){
            ...
            if (($limitType == 0 && $leftTotalCount > 0) || ($limitType == 1 && $leftDayCount > 0)){
                ...
                pdo_insert($this -> table_redpacket_firend, $data);
                $updatedata = array(
    ...
    public function cardbg(){
        $img_url = './source/modules/redpacket/images/card_back.png';
        return $img_url;
        }
    public function doMobileAwards()
    {
        ...
        if (!empty($user)){
            ...
            $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename($this -> table_redpacket_user) . " WHERE income>:income", array(
                    ":income" => $user['income']
                    ));
            $userSort = $total + 1;
            }
    ...
    public function doMobileauth2()
    {
        ...
        $au = $_GPC['au'];
        $code = $_GPC['code'];
        $tokenInfo = $this -> getAuthTokenInfo($code, $_W['weid']);
        $pid = $_GPC['pid'];
        ...
        if ($au == "firend"){
            $url = $_W['siteroot'] . $this -> createMobileUrl('firend', array(
                    'pid' => $pid,
                    "openid" => $openid,
                    "uid" => $_GPC['uid'],
                    "access_token" => $accessToken
    ...
    public function doMobileauth()
    {
        ...
        $pid = $_GPC['pid'];
        $uid = $_GPC['uid'];
        $redirect_uri = $_W['siteroot'] . $this -> createMobileUrl('auth2', array(
                'au' => $au,
                'pid' => $pid,
                'uid' => $uid
                ));
        ...
        $setting = $this -> findAuthSetting($_W['weid']);
        if (!empty($setting) && !empty($setting['appid']) && !empty($setting['secret'])){
            $appid = $setting['appid'];
    ...
    public function getAccessToken()
    {
        ...
        if (!empty($accessToken)){
            ...
            if (TIMESTAMP - $accessToken['createtime'] >= $expires_in){
