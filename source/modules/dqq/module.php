...
class DqqModule extends WeModule {
	...
	public function fieldsFormDisplay($rid = 0) {
		...
		include $this->template('dqq/form');
	}
	...
	public function doAwardlist() {
		...
		if (checksubmit('delete')) {
			...
			message('删除成功！', create_url('index/module', array('do' => 'awardlist', 'name' => 'dqq', 'id' => $id, 'page' => $_GPC['page'])));
		}
		if (!empty($_GPC['wid'])) {
			$wid = intval($_GPC['wid']);
			pdo_update('dqq_winner', array('status' => intval($_GPC['status'])), array('id' => $wid));
			message('标识领奖成功！', create_url('index/module', array('do' => 'awardlist', 'name' => 'dqq', 'id' => $id, 'page' => $_GPC['page'])));
		}
		...
		$sql = "SELECT a.id, a.award, a.description, a.status, a.createtime, b.realname, b.mobile, b.qq FROM ".tablename('dqq_winner')." AS a
		...
		if (!empty($list)) {
			$total = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('dqq_winner')." AS a
				LEFT JOIN ".tablename('fans')." AS b ON a.from_user = b.from_user WHERE a.rid = '$id' $where");
			$pager = pagination($total, $pindex, $psize);
		}
		include $this->template('dqq/awardlist');
	}
