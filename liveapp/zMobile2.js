...
(function () {
    ...
    function TouchBind(IEEventName, OtherEventName, PCName) {
        return function (el, response, isCapture) {
            var mobile = bindEvent(el, window.navigator.msPointerEnabled ? IEEventName : OtherEventName, response);
            var pc = ua.win32 ? bindEvent(el, PCName, response, isCapture) : null;
            return {
                remove: function () {
                    mobile.remove();
                    pc && pc.remove();
